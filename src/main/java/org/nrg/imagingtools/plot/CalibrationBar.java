...
public class CalibrationBar {
   	...
   	*/
   	    ...
   	    public CalibrationBar(ImagePlus img, double min, double max) {
   	    	if (min != -1 && max != -1) {
	   	    	double range = max - min;
	   	    	if (numLabels > range) {
	   	    		numLabels = (int)range + 1;
	   	    	}
   	    	}
   	    	this.imp = img;
   	    	impData = imp;
   	    ...
   	    public CalibrationBar(ImagePlus img) {
   	    	this(img, -1,-1);
   	    }

   	    
   	    public ImagePlus insertColorBar() {
        	//add an extra width at the edge of the image 
        	//Fill with black
        	//Add the color bar
	        run();
	        ImagePlus rtn = new ImagePlus("ColorBar Image",ip);
	       // rtn.getProcessor().setColorModel(imp.getProcessor().getColorModel());
	       // rtn.show();
	        return rtn;
   	    }
   	    ...
   	    public void run() {
   	        ...
   	        if (zoom<=1 && mag<1)
   	            zoom = (double) 1.0/mag;
   	        ip = imp.getProcessor().duplicate().convertToRGB();
   	        impOriginal = imp;
